...
## Description
The shared source repository for LibOrbitUI, LibOrbitColorPicker and LibOrbitGlow. Each library retains its own runtime API, license and independently versioned release.
...
The three path-filtered workflows run on relevant changes pushed to `main` and share release tooling. Tags use `<Library>-<MAJOR.MINOR>`; versions continue from the previous repositories. The first monorepo releases are LibOrbitUI 1.1, LibOrbitColorPicker 1.2 and LibOrbitGlow 1.8. Package versions are independent of runtime API revisions.
...
## Gotchas
- LibOrbitUI exports a private API per embedding addon; ColorPicker and Glow use LibStub's shared versioned instances. Consolidating source does not merge their runtime ownership.
