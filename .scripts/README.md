...
The helper verifies the complete runtime file set, Lua 5.1/XML/TOC load closure, licenses, artwork and the pinned LibStub external before any upload, then normalizes ZIP ordering, timestamps and modes for reproducible retries. SHA256 sidecars accompany every ZIP. Glow's packaged TOC supplies both `release.json` and CurseForge game versions through `game_metadata`; `curse_version_ids` requires an exact version and client-family match before upload. `source.json` identifies the source commit and runtime subdirectory.
...
- Glow metadata supports Retail 12.x and Forever 1.6x; missing or ambiguous CurseForge versions abort before reserving an upload attempt, with no fallback to another client or patch. Run `python .scripts/test-release-metadata.py` for offline publication-metadata checks.
